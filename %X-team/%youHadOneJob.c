...
#pragma config(Motor,  port3,           Lt1,           tmotorVex393HighSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port4,           Lt2,           tmotorVex393HighSpeed_MC29, openLoop, reversed)
...
#pragma config(Motor,  port7,           Lt3,           tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port8,           Lt4,           tmotorVex393HighSpeed_MC29, openLoop)
...
#define straightBtn			Btn8D
#define hyperExtendBtn	Btn8R
#define narutoBtn				Btn8L
	//#endsubregion
	//#subregion lift
...
void clawControl() {
	if (vexRT[openClawBtn]==1 && currentState!=OPEN)
		...
		setClawState(CLOSED);
	else if (vexRT[straightBtn]==1 && currentState!=STRAIGHT)
		setClawState(STRAIGHT);
	else if (vexRT[hyperExtendBtn]==1 && currentState!=HYPEREXTENDED)
		setClawState(HYPEREXTENDED);
	else if (vexRT[narutoBtn]==1 && currentState!=NARUTO)
		setClawState(NARUTO);
...
task skillz() {
	...
	liftTo(MIDDLE);
	setClawState(OPEN);
	driveStraight(-13);	//prepares for first loading
	liftTo(BOTTOM);

	grabNdump(3000);

	for (int i=0; i<2; i++) { //dump preload pillows (x2)
		ramToRealign();

		driveStraight(fenceToWallDist);	//returns to wall
	...
	driveStraight(6, true);	//drives away from fence
	waitForMovementToFinish(false);
	turn(-55, true, 40, 90, -30);	//turns toward center jacks
	waitForMovementToFinish();
	liftTo(BOTTOM);
	driveStraight(40);	//pick up center jacks
	moveClawTo(CLOSED);
	wait1Msec(500);
	setTargetPosition(lift, liftPositions[MIDDLE]+250);
	driveStraight(-6);	//drive to middle of fence
	turnDriveDump(68, -5, 0, 40, 95, -30);	//dump center jacks
	...
	setLiftState(BOTTOM);
	waitForMovementToFinish();
	driveStraight(13);	//drive to central pillow
	moveClawTo(CLOSED);
	turnDriveDump(0, -15);	//dump central pillow

	//grab and dump center back jacks
	setClawState(STRAIGHT);
	ramToRealign();
	driveStraight(fenceToWallDist+2, true);	//drive to back jacks
	waitForMovementToFinish();
	grabNdump(0, fenceToWallDist+10, 750);	//dump back jacks

	//get and dump right side pillow
	ramToRealign();
	driveStraight(3);	//drive away from fence
	turn(-17, true);	//turn toward right side pillow
	waitForMovementToFinish();
	driveStraight(50);	//drive to right side pillow
	moveClawTo(CLOSED);
	turnDriveDump(22, -fenceToWallDist, 10);	//dump pillow

	//get first right side jack
	ramToRealign();
	setLiftState(MIDDLE);
	driveStraight(5, true);	//drive away from fence
	waitForMovementToFinish();
	turn(-55);	//turn to jack
	liftTo(BOTTOM);
	driveStraight(10);	//drive to jack

	//get second side jack
	turn(70);	//turn toward jack
	driveStraight(fenceToWallDist);	//drive to jack
	grabNdump(0);
