...
#pragma config(Motor,  port2,           clawL,         tmotorVex393_MC29, openLoop)
...
#pragma config(Motor,  port6,           rd,            tmotorVex393_MC29, openLoop, reversed)
...
//#region positions
#define liftBottom 735 //lift -- NOTE: if reversed pos = (4096 - val), parentheses necessary
#define liftMiddle 912
#define liftTop 1150
#define liftThrowPos 4096 //1940
#define liftMax 2225
#define clawClosedPos 1625 //claw
#define clawOpenPos 2095
#define clawMax 2640
//#endregion
...
void pre_auton() {
	...
	setDriveMotors(drive, 4, ld1, ld2, rd, rd);
	...
	initializePID(clawPID, potentiometerVal(rightClaw)+clawDiff, 0.2, 0, 0.7, 25);
}

//#region lift
void liftControl() {
	lift.stillSpeed = liftStillSpeed * (potentiometerVal(lift)<liftMiddle ? liftDownSSFactor : 1);
...
//#region claw
void matchClaws() {
	clawPID.target = potentiometerVal(rightClaw) + clawDiff;
	setPower(leftClaw, PID_runtime(clawPID, potentiometerVal(leftClaw)));
}

void clawControl() {
	if (vexRT[openClawBtn] == 1) {
		setPower(leftClaw, 127);
		setPower(rightClaw, 127);
		clawOpen = true;
		...
		clawOpen = true;
		if (potentiometerVal(rightClaw) < clawOpenPos) setPower(rightClaw, 127);

		if (potentiometerVal(leftClaw) < clawClosedPos) {
			setPower(leftClaw, 127);
		} else {
			matchClaws();
		}
...
//			break;
//		case OPEN:
//			createManeuver(claw, clawOpenPos, clawOpenSSFactor * clawStillSpeed, power);
//			clawOpen = true;
//			break;
//		case HYPEREXTENDED:
//			createManeuver(claw, clawMax, clawOpenSSFactor * clawStillSpeed, power);
//			clawOpen = true;
//			break;
//	}
//}
...
//	goToPosition(claw, clawOpenPos, (stillSpeed ? clawOpenSSFactor * clawStillSpeed : 0), power);
...
//	goToPosition(claw, clawMax, (stillSpeed ? clawOpenSSFactor * clawStillSpeed : 0));

//	clawOpen = true;
//}

//void createLiftManeuver(liftState state, int power=127, bool stillSpeed=true) {
//	liftDown = false;

//	int stillSpeedVal = 0;

//	if (stillSpeed) stillSpeedVal = liftStillSpeed * (state==BOTTOM ? liftDownSSFactor : 1);

//	switch (state) {
//		case BOTTOM:
//			createManeuver(lift, liftBottom, stillSpeedVal, power);
//			liftDown = true;
//			break;
//		case MIDDLE:
//			createManeuver(lift, liftMiddle, stillSpeedVal, power);
//			break;
//		case TOP:
//			createManeuver(lift, liftTop, stillSpeedVal, power);
//			break;
//		case THROW:
//			createManeuver(lift, liftThrowPos, stillSpeedVal, power);
//			break;
//		case MAX:
//			createManeuver(lift, liftMax, stillSpeedVal, power);
//			break;
//	}
//}
...
//	switch (state) {
//		case BOTTOM:
//			goToPosition(lift, liftBottom, liftDownSSFactor * liftStillSpeed, power);
//			liftDown = true;
//			break;
//		case MIDDLE:
...
//	createLiftManeuver(TOP, 127, true);
//	driveStraight(10.5, true);
//	while (driveData.isDriving) maneuvers();
//	turn(autoSign * 52, true, 40, 80, -10); //turn to face fence
//	while (turnData.isTurning) maneuvers();
//	driveStraight(28, true); // drive up to wall
...
task autonomous() {
	...
	autoSign = (sidePos < 1820) ? 1 : -1;

	//start appropriate autonomous task
	if (sidePos>1190 && sidePos<2520) {
		startTask(skillz);
	} else if (SensorValue[modePot] > 2525) {
		startTask(blockingAuton);
	} else if (SensorValue[modePot] > 1110) {
